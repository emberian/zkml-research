#!/usr/bin/env python3
"""Apply-check initialized-reference proofs without modifying pinned predecessors."""
...
dependencies = [
    ...
    HERE.parent / 'cse_structure/Compiler/CseStructure.lean',
]
...
patch += ''.join(difflib.unified_diff([], SOURCE.read_text().splitlines(True),
    fromfile='/dev/null', tofile='b/Compiler/CseInitializedReferences.lean'))
patch_path = HERE / 'minidregg-cse-initialized-references.patch'
patch_path.write_text(patch)
scratch = Path(tempfile.mkdtemp(prefix='cse_initialized_package_'))
records = []
...
assert sha(scratch / 'Compiler/CseInitializedReferences.lean') == sha(SOURCE)
...
report = dict(all_package_checks_passed=True, source_sha256=sha(SOURCE), patch_sha256=sha(patch_path),
    ...
    actual_initialized_kernel_proved=True, option_lookup_kernel_proved=True,
    rust_refinement_proved=False, frozen_files_unchanged=True, frozen_file_count=len(after_files),
    ...
    reused_source_sha256={str(COMPANION / 'Compiler' / (n + '.lean')):
        sha(COMPANION / 'Compiler' / (n + '.lean')) for n in ['AirFlatten', 'Emit', 'EmitShare', 'DescriptorEval']},
    scope='Kernel-checked CSE preservation of ForwardValid, every emitted F2 source descriptor '
        'initialized, actual Learn/Infer validCheck theorems, and Some lookup after arbitrary '
        'payload writes in a functional Option-store model. No gate evaluator or Rust/TFHE '
        'refinement. Four fresh proposal dependency oleans with cached external dependencies; '
        'root owns full umbrella build.')
(RESULTS / 'verification.json').write_text(json.dumps(report, indent=2)+'\n')
entry = dict(name='cse_initialized_references', root='formal/cse_initialized_references',
    patch='formal/cse_initialized_references/minidregg-cse-initialized-references.patch', expected_pins=29,
    depends_on=['private_address_ema', 'private_address_ema_schedule', 'emitted_schedule_execution', 'cse_structure'])
