...
if not config_path.exists():
    scratch = Path(tempfile.mkdtemp(prefix='emitted_execution_'))
...
while (RESULTS / f'lean_{index:03d}.json').exists():
    index += 1
command = [config['lean'], '-o', str(Path(config['overlay']) / 'Compiler/EmittedScheduleExecution.olean'), str(source)]
