...
prior=json.loads((p.parent.parent/'formal/results/RangeKeyswitchInfer_003.json').read_text())
env=dict(os.environ,LEAN_PATH=str(p/'build')+':'+str(p.parent.parent/'backend/build')+':'+prior['LEAN_PATH'])
for srcdir in [p.parent/'build/Compiler',p.parent.parent/'backend/build/Compiler']:
 for f in srcdir.glob('*.olean'):
  dst=p/'build/Compiler'/f.name
  dst.parent.mkdir(parents=True,exist_ok=True)
  if not dst.exists(): dst.symlink_to(f.resolve())
for srcroot in [Path('/Users/ember/dev/minidregg/.lake/build/lib/lean'),Path('/tmp/minidregg-full-ud-babybear-20260908/.lake/build/lib/lean')]:
 for namespace in ['Selvage','Theory']:
  for f in (srcroot/namespace).glob('*.olean'):
   dst=p/'build'/namespace/f.name
   dst.parent.mkdir(parents=True,exist_ok=True)
   if not dst.exists(): dst.symlink_to(f.resolve())
