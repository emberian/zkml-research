#!/usr/bin/env python3
"""Fresh isolated compilation of the complete source-certificate dependency chain.
...
lines.insert(i+1,'import Compiler.FheSourceCertificateVerifier  -- exact pinned RNS source certificate; protocol/limb refinement remains open\nimport Compiler.FheSourceCertificateWitness\n');new=''.join(lines)
...
patchpath=ROOT/'minidregg-fhe-source-certificate.patch';patchpath.write_text(patch)
...
run(['git','apply','--check',ROOT.parent/'minidregg-bfv-lift-refinement.patch',ROOT.parent/'engine_refinement/minidregg-fhe-rns-scale-decomposition.patch',patchpath])
assert before=={str(p):sha(p) for p in DEPS+sources},'Source changed during validation'
summary=dict(label='EXECUTED fresh isolated dependency chain plus side-effect-free Compiler umbrella; no full Minidregg build',exact_axiom_pins=pins,total_pins=23,sources_sha256=before,patch_sha256=sha(patchpath),all_commands_passed=True,not_claimed=['universal emitted-layout completeness','whole concrete checker kernel witness','committed-source/proof-protocol binding','constrained target-limb-array projection','Rust Shoup/NTT refinement','private release or master-read absence'])
