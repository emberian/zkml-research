...
PIPELINE = BASE / 'update_pipeline/run.py'
...
def pins():
    paths = [Path(__file__), JOURNAL / 'service.py', LEARNER / 'learner.py', LEARNER / 'encoder.py',
             LEARNER / 'config.json', PIPELINE, PIPELINE.parent / 'EmitBfvExpiryFast.lean', BINARY, NATIVE, TEMPLATE]
    return {str(p): j.sha(p.read_bytes()) for p in paths}
