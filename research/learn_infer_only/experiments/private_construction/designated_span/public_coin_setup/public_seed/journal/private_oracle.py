...
def prepare(root):
 ...
 for n in range(1,34):write(private/f'input-{n:02d}.json',[((n+3)*(j+5)%17)-8 for j in range(577)])
 return {'ok':True,'known_fixture_vectors':33,'dimension':577}
def compare(root):
 ...
 for n in range(1,34):
  ...
  if n in {1:0,16:1,32:2,33:3}:expected[f'infer-{n:02d}']=sum(sum(v[j] for v in queue)*rows[{1:0,16:1,32:2,33:3}[n]][j] for j in range(577))
