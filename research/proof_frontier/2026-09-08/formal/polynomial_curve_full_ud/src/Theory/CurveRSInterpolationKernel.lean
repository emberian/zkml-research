...
AffineLines/GoodCoeffs.lean:212,464. Degree-M curve extension following BCIKS20 §6.1. Direct polynomial-matrix interface;
all coding hypotheses are supplied by the existing Selvage RS layer.
-/
import Theory.CurveRSInterpolationMatrix
...
the formal input is any polynomial word of the stated degree. -/
def CurveSpecializationKernels (e d : ℕ) (ωs : ι → F) (f : ι → F[X]) (S : Finset F) : Prop :=
  ∀ z ∈ S, ∃ a : Fin (e+1) → F, ∃ b : Fin (e+d) → F,
    a ≠ 0 ∧ (BW_homMatrix e d ωs (fun i => (f i).eval z)).mulVec
      (Fin.append a b) = 0

/-- More specializations than the determinant degree force every maximal
minor to vanish; arbitrary row maps are allowed. -/
theorem curve_det_eq_zero_of_specializations
    (cdeg e deg : ℕ) (domain : ι ↪ F) (f : ι → F[X]) (S : Finset F)
    (hfdeg : ∀ i, (f i).natDegree ≤ cdeg)
    (hS : cdeg*(e+1) < S.card)
    (hgood : CurveSpecializationKernels e deg domain f S)
    (r : Fin ((e+1)+(e+deg)) → ι) :
    Matrix.det ((BW_homMatrix e deg (fun i => (Polynomial.C (domain i) : F[X]))
      f).submatrix r id) = 0 := by
  classical
  let N : ℕ := (e + 1) + (e + deg)
  ...
  have hdeg_det : (Matrix.det A).natDegree ≤ cdeg*(e + 1) := by
    simpa [A, M, N] using
      (BW_det_natDegree_le cdeg e deg domain f hfdeg
        (r := (r : Fin N → ι)))
  have he1_lt : cdeg*(e+1) < S.card := hS
  ...
  have heval : ∀ z ∈ S, (Matrix.det A).eval z = 0 := by
    ...
    let Mz : Matrix ι (Fin N) F :=
      BW_homMatrix (ι := ι) e deg (fun i => domain i) (fun i => (f i).eval z)
...
theorem curve_bounded_kernel_of_specializations
    (cdeg e deg : ℕ) (domain : ι ↪ F) (f : ι → F[X]) (S : Finset F)
    (hfdeg : ∀ i, (f i).natDegree ≤ cdeg)
    (hdegpos : 1 ≤ deg) (hradius : 2*e+deg ≤ Fintype.card ι)
    (hScard : cdeg*(e+1) < S.card)
    (hgood : CurveSpecializationKernels e deg domain f S) :
    ∃ a : Fin (e+1) → F[X], ∃ b : Fin (e+deg) → F[X],
      a ≠ 0 ∧ (∀ t, (a t).natDegree ≤ cdeg*e) ∧
      (∀ s, (b s).natDegree ≤ cdeg*(e+1)) ∧
  ...
  have hdetM : ∀ r : Fin N → ι, Matrix.det (Matrix.submatrix M r id) = 0 := by
    intro r
    simpa [M, N, m, n] using
      curve_det_eq_zero_of_specializations cdeg e deg domain f S hfdeg hScard hgood r
  ...
  have hdegL : ∀ i j, (L i j).natDegree ≤ cdeg := by
    intro i j
    simpa [L, cL, M] using
      (BW_entry_natDegree_le cdeg e deg domain f hfdeg i (cL j))
  have hdegA21 : ∀ i j, (A21 i j).natDegree ≤ cdeg := by
    intro i j
    simpa [A21, cL, M] using
      (BW_entry_natDegree_le cdeg e deg domain f hfdeg (rB i) (cL j))
  have hdegR0 : ∀ i j, (R i j).natDegree = 0 := by
    intro i j
    have hj : e + 1 ≤ (cR j).1 := by
      simp [cR, m]
    simpa [R, cR, M] using
      (BW_entry_natDegree_zero e deg domain f i (cR j) hj)
  ...
  have hdegInvDA21 : ∀ i j, ((⅟D * A21) i j).natDegree ≤ cdeg := by
    intro i j
    classical
    have hterm : ∀ k ∈ (Finset.univ : Finset (Fin n)),
        ((⅟D i k) * (A21 k j)).natDegree ≤ cdeg := by
      intro k hk
    ...
    have hsum :=
      Polynomial.natDegree_sum_le_of_forall_le (s := (Finset.univ : Finset (Fin n)))
        (n := cdeg) (f := fun k : Fin n => (⅟D i k) * (A21 k j)) hterm
    simpa [Matrix.mul_apply] using hsum
  have hdegRInvDA21 : ∀ i j, ((R * (⅟D * A21)) i j).natDegree ≤ cdeg := by
    intro i j
    classical
    have hterm : ∀ k ∈ (Finset.univ : Finset (Fin n)),
        (R i k * ((⅟D * A21) k j)).natDegree ≤ cdeg := by
      intro k hk
      ...
      calc
        ...
        _ ≤ cdeg := hdegInvDA21 k j
    have hsum :=
      Polynomial.natDegree_sum_le_of_forall_le (s := (Finset.univ : Finset (Fin n)))
        (n := cdeg) (f := fun k : Fin n => R i k * ((⅟D * A21) k j)) hterm
    simpa [Matrix.mul_apply] using hsum
  have hdegK0 : ∀ i j, (K0 i j).natDegree ≤ cdeg := by
    intro i j
    ...
    have hmax :
        max (L i j).natDegree ((R * (⅟D * A21)) i j).natDegree ≤ cdeg := by
  ...
  obtain ⟨a, ha0, ha_deg, haKer⟩ :=
    PolynomialMatrixKernel.Source.RS_exists_nonzero_kernelVec_of_det_submatrix_eq_zero_natDegree_le (ι := ι) (F := F) cdeg e K0
  ...
  · intro s
    classical
    have hdegA21mulVec : ∀ i : Fin n, ((A21.mulVec a) i).natDegree ≤ cdeg*(e + 1) := by
      intro i
      have hterm : ∀ t ∈ (Finset.univ : Finset (Fin m)),
          (A21 i t * a t).natDegree ≤ cdeg*(e + 1) := by
        intro t ht
        have hA : (A21 i t).natDegree ≤ cdeg := hdegA21 i t
        have ha : (a t).natDegree ≤ cdeg*e := ha_deg t
        have hmul : (A21 i t * a t).natDegree ≤ (A21 i t).natDegree + (a t).natDegree :=
          Polynomial.natDegree_mul_le
        have hadd : (A21 i t).natDegree + (a t).natDegree ≤ cdeg + cdeg*e := Nat.add_le_add hA ha
        have : (A21 i t * a t).natDegree ≤ cdeg + cdeg*e := le_trans hmul hadd
        simpa [Nat.mul_add, Nat.add_comm] using this
      have hsum :=
        Polynomial.natDegree_sum_le_of_forall_le (s := (Finset.univ : Finset (Fin m)))
          (n := cdeg*(e + 1)) (f := fun t : Fin m => A21 i t * a t) hterm
      simpa [Matrix.mulVec, dotProduct] using hsum
    have hdegInvDmulVec : ∀ i : Fin n, ((⅟D).mulVec (A21.mulVec a) i).natDegree ≤ cdeg*(e + 1) := by
      intro i
      have hterm : ∀ k ∈ (Finset.univ : Finset (Fin n)),
          ((⅟D i k) * (A21.mulVec a k)).natDegree ≤ cdeg*(e + 1) := by
        intro k hk
        have hInv' : (D⁻¹ i k).natDegree = 0 := hdegInvD0 i k
        have hA : ((A21.mulVec a) k).natDegree ≤ cdeg*(e + 1) := hdegA21mulVec k
        calc
      ...
      have hsum :=
        Polynomial.natDegree_sum_le_of_forall_le (s := (Finset.univ : Finset (Fin n)))
          (n := cdeg*(e + 1)) (f := fun k : Fin n => (⅟D i k) * (A21.mulVec a k)) hterm
      simpa [Matrix.mulVec, dotProduct] using hsum
...
/-- info: 'Minidregg.RSInterpolation.curve_det_eq_zero_of_specializations' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Minidregg.RSInterpolation.curve_det_eq_zero_of_specializations

/-- info: 'Minidregg.RSInterpolation.curve_bounded_kernel_of_specializations' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Minidregg.RSInterpolation.curve_bounded_kernel_of_specializations
