...
import flint
from ring import Parameters, PublicSetup, Window, Ring, uniform_below, generate_recipient_key, RecipientBatch
...
class MeteredSampler:
    ...
    def sample(self, sigma, count):
        ...
        if 'native_calls' in self.inner.last_stats:
            totals['native_calls']=totals.get('native_calls',0)+self.inner.last_stats['native_calls']
        return out
...
def execute(profile, inputs):
    ...
    readers, timings['recipient_batch_prepare'] = timed(lambda: RecipientBatch(setup,keys))
    ...
    for step in range(inputs):
        ...
        decoded, dec=timed(lambda: readers.decode(ct))
        if profile=='toy':
            assert decoded==[setup.decode(key,ct) for key in keys]
        assert decoded == transformed[:p.recipients]
        cts.append(ct); lifts.append(transformed)
        current, upd=timed(lambda: window.push(ct))
        got, win_dec=timed(lambda: readers.decode(current))
    ...
    signed_values, timings['signed_all_recipient_decode'] = timed(lambda: readers.decode(signed))
    ...
    result={'profile':profile,'status':'PASS','parameters':p.__dict__,
       'decoder':'shared ciphertext extraction plus exact FLINT integer-matrix product',
       'native_sampler':{'library_path':str(sampler.inner.library_path),
          'library_sha256':hashlib.sha256(Path(sampler.inner.library_path).read_bytes()).hexdigest(),
          'build_command':sampler.inner.build_command},
...
def main():
    ...
    sources=['ring.py','run.py','sampling/sampler.py','sampling/proposal_screen.c','sampling/reference.py','sampling/__init__.py']
