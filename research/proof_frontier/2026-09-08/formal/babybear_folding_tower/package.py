...
repo=Path('/tmp/minidregg-babybear-folding-tower-20260908')
research=Path('/Users/ember/dev/zkml-research/research')
name='babybear_folding_tower'
...
patch=out/'babybear-folding-tower.patch';patch.write_text(''.join(patches))
with tempfile.TemporaryDirectory(prefix='babybear-folding-tower-apply-')as td:
...
(out/'logs/patch-apply.log').write_text('PASS: clean git apply --check, git apply, exact byte equality for all four source files.\n')
