...
[EXECUTED completed successors] The [source certificate23](formal/bfv_lift_refinement/source_certificate/REPORT.md) forces the deterministic Garner correction in the existing compiler; [target projection22](formal/bfv_lift_refinement/target_projection/REPORT.md) binds the same integer to all three canonical residues. [Modular words38](formal/bfv_lift_refinement/modular_lowering/REPORT.md) then connects that checker to the complete handwritten Shoup/Barrett/correction source-word model without adding constraints. All are in the final651-pin collection. Actual API/row fixtures pass; Rust/compiler/array semantics and operand/NTT provenance remain separate. The proposed operand-provenance follow-on did not reach a completed artifact before interruption.
