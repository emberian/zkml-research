...
s=(P.parent/'full_bfv_tensor_arithmetic/Compiler/BfvTensorMul.lean').read_text()
nums={11:6,6:9,64:512,22:12,23:13,12:16,2048:32768,131072:16777216,121:36,3969:261121,63:511,4095:65535,4096:65536}
s=re.sub(r'\b\d+\b',lambda m:str(nums.get(int(m[0]),int(m[0]))),s).replace('BfvTensorMul','BfvKeyswitchMac')
a=s.index('theorem canonicalSystem_sound');b=s.index('theorem column_correct');c=s.index('theorem radix_balance');d=s.index('theorem rowSystem_sound')
helpers=s[a:b]+s[c:d]
header='''/- Source-native four-term modular multiply-accumulate for actual BFV switching. -/
...
theorem products_denote (a b : Fin 4 → Fin 6 → Nat) :
  ...
  simp only [products,Fin.sum_univ_succ]
  simp only [← convolution_denote]
  simp only [← denote_add]
  congr 2
  funext i
  simp
...
theorem column_correct {J : Type} (q : Nat) (w : RowWires J) (asg : J → BabyBear) (t : Fin 12) :
  ...
  simp [column,accepts,eval_add',eval_mul',eval_cst,eval_vr,productTerm_eval,convolutionTerm_eval,padded]
  ring_nf
...
'''
(P/'Compiler/BfvKeyswitchMac.lean').write_text(header+helpers+new)
