...
use std::{collections::BTreeMap, error::Error, fs::{self,OpenOptions}, io::{Read,Write},
    os::unix::fs::{OpenOptionsExt,PermissionsExt}, sync::Arc, time::Instant};
        ...
        Err(e) if e.kind()==std::io::ErrorKind::AlreadyExists=>{
            if secret{
                ...
                if !m.file_type().is_file() || m.permissions().mode() & 0o077 != 0{
                    return Err(err("existing secret output must be a private regular file (no symlink/group/other permissions)"));
                }
            }
...
mod tests{
    use super::*;
    #[test]
    fn secret_output_retry_requires_private_regular_file(){
        let dir=std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("runs")
            .join(format!("secret-save-test-{}-{}",std::process::id(),
                std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        fs::create_dir_all(&dir).unwrap();
        let file=dir.join("private");let path=file.to_str().unwrap();
        save(path,b"noncredential-test-bytes",true).unwrap();
        assert_eq!(fs::metadata(&file).unwrap().permissions().mode()&0o777,0o600);
        save(path,b"noncredential-test-bytes",true).unwrap();
        fs::set_permissions(&file,fs::Permissions::from_mode(0o644)).unwrap();
        assert!(save(path,b"noncredential-test-bytes",true).unwrap_err().to_string().contains("private regular"));
        fs::set_permissions(&file,fs::Permissions::from_mode(0o600)).unwrap();
        let link=dir.join("link");std::os::unix::fs::symlink(&file,&link).unwrap();
        assert!(save(link.to_str().unwrap(),b"noncredential-test-bytes",true).unwrap_err().to_string().contains("private regular"));
        save(path,b"noncredential-test-bytes",true).unwrap();
    }
}
