...
patch=out/'coherent-tail.patch';chunks=[]
...
replay=Path(tempfile.mkdtemp(prefix='coherent-tail-replay-'))
...
pred=research/'commitment_failure_successor';pm=json.loads((pred/'MANIFEST.json').read_text())
for source_package in ['constructive_successor','multi_round_successor','finite_schedule_successor','commitment_failure_successor']:
...
evidence += json.loads((out/'SOURCES.json').read_text())['primary_sources']
evidence += [{'path':str(out/'query_transport/MANIFEST.json'),'sha256':sha(out/'query_transport/MANIFEST.json')},{'path':str(out/'query_transport/checks/frozen-dependency-refresh.json'),'sha256':sha(out/'query_transport/checks/frozen-dependency-refresh.json')}]
for row in evidence:assert sha(Path(row['path']))==row['sha256']
entry={'name':'coherent_tail_successor','root':'../proof_frontier/2026-09-08/coherent_tail_successor','patch':'../proof_frontier/2026-09-08/coherent_tail_successor/coherent-tail.patch','expected_pins':sum(x['guards']for x in modules),'source_files':{str(m):'../proof_frontier/2026-09-08/coherent_tail_successor/src/'+str(m)for m in sorted(mods)}}
save(out/'integration_entry.json',entry)
manifest={'scope':'cumulative weighted-agreement invariant and exact actual coherent-query identity; five-round BabyBear ideal/supplied/fresh-query soundness with tail (4/5)^q','base_commit':base,'checkout':str(repo),'lean_toolchain':(repo/'lean-toolchain').read_text().strip(),'modules':modules,'module_count':len(modules),'theorem_count':sum(m['theorems']for m in modules),'all_exact_guarded_checks_pass':True,'standard_axioms_only':True,'patch':patch.name,'patch_sha256':sha(patch),'dependencies':deps,'source_evidence':evidence,'import_gate':ig,'patch_replay':pr,'broad_rebuild_performed':False,'independent_review_campaign_performed':False,'new_web_queries':2,'new_scry_sql_queries':0,'predecessors_preserved':True,'artifact_sha256':{str(p.relative_to(out)):sha(p)for p in [out/'README.md',out/'STATUS.md',out/'NEXT.md',out/'DEPENDENCIES.json',out/'declaration-names.json',out/'integration_entry.json',out/'run_lean.py',out/'pin_all.py',out/'finalize.py',out/'final_checks.py',out/'TARGET.md',out/'SOURCES.json']}}
manifest['query_transport_builder_manifest']={'path':str(out/'query_transport/MANIFEST.json'),'sha256':sha(out/'query_transport/MANIFEST.json')}
