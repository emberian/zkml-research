...
def verify_dependencies():
 ...
 for name,item in pins['files'].items():
  require(sha((HERE/name).read_bytes())==item['sha256'],'immutableDependency:'+name)
  require(oct(stat.S_IMODE((HERE/name).stat().st_mode))==item['mode'],'immutableDependencyMode:'+name)
 return pins

def verify_binding(root):
 root=Path(root);g=read_json(root/'genesis.json');b=g['public_seed_setup']
 require(b['schema']=='public-seed-journal-genesis-binding-v1','publicSetupBindingSchema')
 for name,expected in b['public_artifacts_sha256'].items():require(sha((root/name).read_bytes())==expected,'boundSetupArtifact:'+name)
 require(b['setup_source_sha256']==sha((SOURCE/'seed_setup/seed_setup.py').read_bytes()),'boundSetupSource')
 ...
 require(t['status']=='accepted' and t['source_identity']==b['setup_source_identity'],'transcriptSourceIdentity')
 require(t['source_identity']['seed_setup_sha256']==b['setup_source_sha256'],'transcriptWrapperIdentity')
 require(t['source_identity']['crypto_sources']==g['crypto_sources'],'transcriptCryptoIdentity')
 require(t['source_identity']['native_dependency']==g['native_dependency'],'transcriptNativeIdentity')
 require(t['source_identity']['manifest_sha256']==b['seed_adapter_manifest_sha256']==sha((SOURCE/'seed_setup/SOURCE_PINS.json').read_bytes()),'nestedSeedSourceIdentity')
 require(b['seed_hex']==t['derivation']['seed_hex'] and b['suite']==t['derivation']['suite']==t['complete_domain']['suite'],'fullSeedRecipeIdentity')
 require(b['rejection_cap']==t['complete_domain']['rejection_cap']==128 and b['complete_domain_sha256']==digest(t['complete_domain']),'completeDomainIdentity')
 require(t['registry_sha256']==digest(reg)==b['public_artifacts_sha256']['registry.json'],'transcriptRegistryIdentity')
 require(v['complete_context_byte_match'] and v['complete_raw_hash_tape_match'] and v['public_inputs_only'] and v['transcript_sha256']==digest(t),'trustedFullPublicVerificationRecord')
 ...
 require(read_json(root/'public_context.json')['rows']==t['complete_domain']['rows'],'completeDomainRows')
 require(sha((root/'public_context.json').read_bytes())==g['context_id'] and sha((root/'context_validation.json').read_bytes())==g['context_validation_sha256'],'boundContextFiles')
 return {'ok':True,'genesis_sha256':digest(g),'public_artifacts_checked':len(b['public_artifacts_sha256']),'public_transcript_sha256':digest(t),'registration_sha256':digest(reg),'complete_domain_sha256':digest(t['complete_domain']),'seed_adapter_manifest_sha256':b['seed_adapter_manifest_sha256'],'trusted_setup_binding_only':True}
...
def setup(root):
 ...
 binary=SOURCE/'crypto/crypto.py';adapter=SOURCE/'seed_setup/seed_setup.py'
 ...
 require(not built['scalar_master_computed'] and not built['scalar_projection_delivery_computed'] and built['public_seed_candidates']==1,'noScalarDealerSetup')
 ...
 transcript=read_json(root/'public_transcript.json')
 bound=['public_context.json','context_validation.json','public_transcript.json','registry.json','public_setup_verification.json',*[f'announcements/r{i:02d}.json' for i in range(16)]]
 setup_binding={'schema':'public-seed-journal-genesis-binding-v1','public_artifacts_sha256':{name:sha((root/name).read_bytes()) for name in bound},'setup_source_sha256':sha(adapter.read_bytes()),'setup_source_identity':transcript['source_identity'],'seed_adapter_manifest_sha256':sha((SOURCE/'seed_setup/SOURCE_PINS.json').read_bytes()),'seed_hex':transcript['derivation']['seed_hex'],'suite':transcript['derivation']['suite'],'rejection_cap':transcript['complete_domain']['rejection_cap'],'complete_domain_sha256':digest(transcript['complete_domain']),'join_source_sha256':sha(Path(__file__).read_bytes()),'dependency_inventory_sha256':sha((HERE/'SOURCE_PINS.json').read_bytes()),'sampling_premise':'one fixed SHAKE256 seed and capped full-domain recipe; classical ROM theorem conditional; public timing not covered','registration_premise':'fresh fixed authenticated slots and independent honest recipient-owned scalars','scalar_master_computed_in_executed_setup_path':False,'scalar_projection_delivery_computed_in_executed_setup_path':False}
 g={'schema':'resident-genesis-v1','crypto':params,'key_id':built['context_id'],'context_id':built['context_id'],'public_key_sha256':sha(pk.read_bytes()),'zero_ct_sha256':sha((root/'zero.ct').read_bytes()),'capacity':32,'routes':[0,1],'issuer_vk':issuer_vk,'command_vk':command_vk,'authority_vk':authority_vk,'recipient':'dedicated-fixed-span-recipient-coalition','query_policy':policy,'query_bindings':bindings,'feature_policy':'trusted-issuer-signed577-int8-bound127-v1','custody':'Concrete fixed-seed setup with honest independent fresh registration; conditional ROM/DDH only. No scalar master/projection delivery executed. All16 recipient-owned scalar keys remain in private drain; full per-input fixed span. Shared OS, no physical isolation or timing claim.','journal_program':'designated-ddh-keyless-recomputation-delta-journal-two-phase-v1','crypto_binary_sha256':base['crypto_binary_sha256'],'crypto_sources':base['crypto_sources'],'native_dependency':base['native_dependency'],'context_validation_sha256':sha((root/'context_validation.json').read_bytes()),'journal_sources':{n:sha((SOURCE/'journal'/n).read_bytes()) for n in CORE_NAMES},'public_seed_setup':setup_binding}
