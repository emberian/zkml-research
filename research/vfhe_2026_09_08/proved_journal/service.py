#!/usr/bin/env python3
"""Local durable BFV update gate over the approved native emitted-relation verifier.
...
def initialize(root, binary, template, initial, recipient, learner_class, learner_event,
    ...
    genesis = {'schema': 'proved-continuing-update-genesis-v1',
        ...
        'learner_class': learner_class, 'initial_learner_event': learner_event, 'initial_acc_sha256': sha(acc),
    ...
    gid = digest(genesis)
    state = {'genesis': gid, 'revision': 0, 'acc_sha256': sha(acc), 'learner_event': learner_event,
...
class Service:
    ...
    def audit_reopen(self):
        ...
        state = {'genesis': self.gid, 'revision': 0, 'acc_sha256': self.genesis['initial_acc_sha256'],
                 'learner_event': self.genesis['initial_learner_event'], 'recipient_id': self.genesis['recipient_id'], 'last_receipt_sha256': None}
        self.cas(state['acc_sha256'])
        ...
        try:
            ...
            for row in db.execute('SELECT * FROM journal ORDER BY revision'):
                ...
                self.policy(request)
                need(request['payloads']['acc'] == state['acc_sha256'] and request['parent_learner_event'] == state['learner_event'], 'retained_ciphertext_parent')
                ...
                need(receipt['next_acc_sha256'] == state['acc_sha256'], 'retained_output_identity')
    ...
    def policy(self, req):
        fields = {'schema', 'request_id', 'genesis', 'parent_revision', 'parent_sha256', 'recipient_id',
                  ...
                  'source_event_sha256', 'learner_class', 'parent_learner_event', 'learner_event'}
        need(set(req) == fields and req['schema'] == 'proved-bfv-expiry-request-v1', 'request_schema')
        ...
        need(req['learner_class'] == self.genesis['learner_class'], 'learner_class')
        ...
        hashes = [req[k] for k in ('genesis', 'parent_sha256', 'template_sha256', 'randomness_rule_sha256',
                  'public_rows_sha256', 'proof_sha256', 'source_event_sha256')] + list(req['payloads'].values())
    ...
    def current_parent(self, state, req):
        ...
        need(req['payloads']['acc'] == state['acc_sha256'] and req['parent_learner_event'] == state['learner_event'], 'current_ciphertext_parent')

    def next_state(self, state, req, receipt_hash):
        return {'genesis': self.gid, 'revision': state['revision'] + 1, 'acc_sha256': req['payloads']['out'],
            ...
            try:
                ...
                for h, raw in pinned.items():
                    ...
                    if path.exists():
                        ...
                        write(path, raw)
                receipt = {'schema': 'proved-bfv-expiry-receipt-v1', 'revision': state['revision'] + 1,
...
def request_bundle(root, case, proof, request_id, output):
    ...
    request = {'schema': 'proved-bfv-expiry-request-v1', 'request_id': request_id, 'genesis': service.gid,
...
def main():
    ...
    for name in ('root', 'binary', 'template', 'initial'):
        init.add_argument('--' + name, type=Path, required=True)
    for name in ('recipient', 'learner-class', 'expected-binary-sha256', 'expected-template-sha256'):
        init.add_argument('--' + name, required=True)
    init.add_argument('--learner-event', type=int, required=True)
    ...
    if args.command == 'init':
        result = initialize(args.root, args.binary, args.template, args.initial, args.recipient, args.learner_class,
                            args.learner_event, args.expected_binary_sha256, args.expected_template_sha256)
