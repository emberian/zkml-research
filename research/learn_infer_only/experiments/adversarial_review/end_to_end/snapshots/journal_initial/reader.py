"""Trusted full-key reader R. Baseline trusts honest authority recomputation.

Authority signing-key compromise can forge a query-shaped release of another Ct;
this is an explicit tested credential boundary, not an untrusted-authority claim.
"""
import argparse
...
class Reader:
 def __init__(self,config):
  ...
  secret=Path(config['bfv_secret_key']).read_bytes();require(sha(secret)==config['bfv_secret_sha256'],'readerPinnedSecretBytes')
  require(len(secret)>81 and secret[:8]==b'RSBFV001' and secret[8]==3,'readerSecretEnvelope')
  require(secret[9:41].hex()==self.g['crypto']['params_id'] and secret[41:73].hex()==self.g['key_id'],'readerPinnedKeyRelation')
  ...
  with connect(config['db']) as db:
   ...
   db.execute('CREATE TABLE IF NOT EXISTS received (nonce TEXT PRIMARY KEY,request_id TEXT UNIQUE NOT NULL,envelope_sha256 TEXT NOT NULL,output_ct TEXT NOT NULL,answer TEXT NOT NULL)')
 ...
 def receive(self,envelope,raw):
  ...
  require(p['delta']=={'kind':'Infer','route':a['route'],'query_ct':a['query_ct'],'output_ct':p['output_ct']},'finalizedDelta')
  self.cas.put(raw,'ct',p['output_ct']);envelope_hash=digest(envelope)
  ...
  try:
   ...
   if prior:
    ...
    answer=self.cas.crypto.run('reader-decrypt',private_output=True,sk=self.cfg['bfv_secret_key'],ct=self.cas.get(p['output_ct']))
    require(answer['key_id']==self.g['key_id'] and answer['ciphertext_sha256']==p['output_ct'],'readerOutputIdentity')
    db.execute('INSERT INTO received VALUES(?,?,?,?,?)',(a['nonce'],a['request_id'],envelope_hash,p['output_ct'],canonical(answer).decode()));status='received'
   db.execute('COMMIT')
  finally:
