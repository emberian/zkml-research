...
COUNTS = {"native_modexp": 0, "subgroup_checks": 0, "context_consistency_rows": 0,
          "full_context_validations": 0, "pinned_context_validation_reuses": 0}
ENGINE = None
VALIDATED_CONTEXT_HASH = None
...
def load_context(path):
    ...
    reuse = VALIDATED_CONTEXT_HASH is not None
    if reuse:
        require(type(VALIDATED_CONTEXT_HASH) is str and len(VALIDATED_CONTEXT_HASH) == 64 and
                all(c in "0123456789abcdef" for c in VALIDATED_CONTEXT_HASH) and sha(raw) == VALIDATED_CONTEXT_HASH,
                "expected previously validated context digest")
    body = parse(raw)
    ...
    COUNTS["pinned_context_validation_reuses" if reuse else "full_context_validations"] += 1
...
def run(args):
    ...
    if args.command == "recipient-finalize":
        ctx = load_context(args.context)
        require(0 <= args.row < K, "recipient row")
    ...
    if args.command == "validate-context":
        require(VALIDATED_CONTEXT_HASH is None, "validation must be uncached")
        return {"validated": True, "validation_mode": "full_subgroups_and_token_consistency",
                "params_id": PARAMS_ID, "context_id": ctx["id"], "key_id": ctx["id"],
                "source_sha256": {name: sha((HERE / name).read_bytes()) for name in ("crypto.py", "group.py", "native_pow.py", "rows.json")},
                "assumptions": "validates public algebra; not honest master generation or erasure"}
...
def main():
    global ENGINE, VALIDATED_CONTEXT_HASH
    parser = argparse.ArgumentParser(description=__doc__, allow_abbrev=False)
    sub = parser.add_subparsers(dest="command", required=True)
    specifications = {
        "params": [], "validate-context": [], "keygen": ["pk", "sk", "zero"],
    ...
    for command, flags in specifications.items():
        ...
        p.add_argument("--validated-context-sha256")
        for flag in flags:
            p.add_argument("--" + flag, required=True, type=int if flag == "row" else str)
        if command == "host-learn":
            p.add_argument("--old")
    args = parser.parse_args()
    VALIDATED_CONTEXT_HASH = args.validated_context_sha256
    start = time.perf_counter_ns()
    try:
        ENGINE = NativePow(P)
        result = run(args)
