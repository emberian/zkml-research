#!/usr/bin/env python3
"""Fresh isolated compilation of the target projection and frozen source dependency chain.
...
lines.insert(i+1,'import Compiler.FheTargetProjectionVerifier  -- canonical target limbs for the pinned source scalar; provenance remains open\nimport Compiler.FheTargetProjectionWitness\n');new=''.join(lines)
...
patchpath=ROOT/'minidregg-fhe-target-projection.patch';patchpath.write_text(patch)
...
run(['git','apply','--check',ROOT.parent/'minidregg-bfv-lift-refinement.patch',ROOT.parent/'engine_refinement/minidregg-fhe-rns-scale-decomposition.patch',ROOT.parent/'source_certificate/minidregg-fhe-source-certificate.patch',patchpath])
assert before=={str(p):sha(p) for p in DEPS+sources},'Source changed during validation'
summary=dict(label='EXECUTED fresh isolated dependency chain plus side-effect-free Compiler umbrella; no full Minidregg build',exact_axiom_pins=pins,total_pins=22,sources_sha256=before,patch_sha256=sha(patchpath),all_commands_passed=True,not_claimed=['universal target emitted-layout completeness','whole concrete composite-checker kernel witness','committed-source/proof-protocol binding','Rust Shoup/NTT refinement','private release or master-read absence'])
