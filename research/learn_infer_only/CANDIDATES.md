...
[DERIVED current comparison, renewed constructive swarm] The actual restricted-key ring construction now runs a continuing semantic learner and durable accepted-state service without constructing a universal reader. Its sixteen recipients retain their complete per-input query span. Separately, a fresh BFV service completes two text teachings, two update proofs, two class-query proofs and all verification before receiving in 21.202 seconds; that path retains a full reader. The entire captured nonlinear nine-to-four rescale is now proved, and native witness execution removes the earlier interpretation bottleneck. Current builders are joining whole BFV multiplication, exact TFHE products, stronger coherent-query soundness and proof-bearing restricted-ring updates. A dual384-bit seeded semantic service has also completed its sole fresh run; its report and conditional QROM application are being sealed. These are distinct constructions; their conjunction is still open. See [current results and exact scopes](../vfhe_2026_09_08/STATUS.md).
