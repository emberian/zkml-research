...
FIELDS = {'schema','request_id','delivery_id','coordinate','genesis','registry_sha256',
          'recipient_set_sha256','setup_descriptor_sha256','registration_sha256','query_sha256','accepted_revision',
...
def validate_identity(service, request):
    ...
    s.need(type(coordinate) is int and 0 <= coordinate < 16, 'registered_recipient_coordinate')
    s.need(request['genesis'] == service.gid and all(request[k] == g[k] for k in ('registry_sha256','recipient_set_sha256','setup_descriptor_sha256')), 'delivery_genesis_registry')
...
def recipient_worker(root, request_path, outdir):
    ...
    s.need(kh['coordinate'] == coordinate and all(kh[k] == service.genesis[k] for k in ('setup_id','registry_sha256','a_sha256','seed_policy','seed_bits')), 'delivery_own_key_context')
    outdir = Path(outdir)
