...
authority, including the authority signing key, may be adversarial. The reader,
input issuer and their private persistence/configuration remain trusted. This
...
public metadata from one valid private vector. An issued input has a unique
identity. Query authorization selects public query bytes, recipient and context.
...
It does not hold across arbitrary rollback or cloning of the reader's own secret
database without an explicit fork-aware ideal interface. A copied authority
database alone does not reset the reader. Availability and progress are not
postconditions of this argument.
...
1. For every high-level sequence of input identities and permitted queries that
   the malicious host/authority can cause the reader to accept, both challenge
   streams have identical prescribed outward outputs. Both remain valid for the
   issuer's range/feature rules. Compatibility covers every accepted continuation,
   not only the benchmark's recorded questions.
...
3. Fresh challenge encryption has the needed multi-message IND-CPA security,
   ...
   histories except with the explicitly bounded endpoint failure probability.
...
authenticated ciphertexts. It simulates scalar delivery using the known left
plaintext semantics at the accepted identity/query sequence. Compatibility
makes that answer correct in both endpoint worlds. It simulates historical
retry, refusal and deduplication from public history, without decrypting arbitrary
adversarial ciphertexts. An unissued input authorization causes the proof's
forgery bad event rather than being silently assigned a known plaintext.
...
[DERIVED bound form] For m challenge input ciphertexts, the distinguishing gap
...
[OPEN review] This root-authored successor has not yet received independent
mathematical review. The previously reviewed honest-authority note remains
unchanged; no stronger result is attributed to that earlier review.
