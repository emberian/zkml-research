...
class CacheFilesTest(unittest.TestCase):
    ...
    def test_noncanonical_export_storage_rejected(self):
        self.object_dir = cache.BUILD_ROOT / '..' / 'synthetic-cache-escape'
        with self.assertRaisesRegex(cache.CacheError, 'Canonical export path required'):
            self.export()
        self.assertFalse(self.metadata.exists())
