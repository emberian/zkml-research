...
claim. See PROVENANCE.md for the seeded-construction / random-oracle obligation.
"""
import hashlib
import time

PROTOCOL = b"ring-seed-semantic/public-rows/v2"
...
class PublicExpander:
    """PublicExpander(seed[48], binding[32]).row(family,index,N,q).
    ...
    def __init__(self, seed: bytes, binding: bytes):
        if not isinstance(seed, bytes) or len(seed) != 48:
            raise ValueError("public seed must be exactly 48 bytes")
