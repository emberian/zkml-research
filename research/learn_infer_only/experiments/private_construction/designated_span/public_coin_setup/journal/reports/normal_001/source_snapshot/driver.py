#!/usr/bin/env python3
"""Normal40/4/8; all public replay and shutdown precede private scalar drain."""
...
WRAPPERS=['driver.py','setup_join.py','private_oracle.py','CONTRACT.md','SOURCE_PINS.json']
...
def prepare(reports):
 ...
 pins={'schema':'public-coin-journal-execution-pins-v1','source_files_sha256':files,'native_dependency':{'path':str(native),'sha256':file_sha(native)},'interpreter':{'path':str(interpreter),'sha256':file_sha(interpreter),'version':sys.version},'signature_dependency':{'version':dist.version,'files_sha256':signature},'platform':platform.platform(),'normal_workload':{'learns':40,'infers':4,'expiries':8},'prepared_only':True}
...
class PublicRun(Run):
 ...
 def role(self,command,**kwargs):
  ...
  return reply['result']
 def reopen(self):
  before=self.head();verified=rpc(self.rcfg['socket'],{'op':'verified_status'});require(before['revision']==verified['revision']==22,'reopenRevision22')
  Run.close(self)
  ...
  return {'revision':22,'state_digest':before['state_digest'],'orderly_close_reopen':True,'exact_head_preserved':True}
 def close(self):
...
def run(root,reports):
 ...
 try:
  ...
  for n in range(1,41):
   ...
   if n%10==0:
    req=instance.prepare('Infer',0,f'infer-{n:02d}',query_index=n//10-1);reply=instance.accepted(req)
    require(reply['status']=='accepted' and reply['delivery']['reader']['status']=='ciphertextAccepted','normalCiphertextAcceptance')
    requests.append(req);accepted.append(reply['envelope']);progress('events',learns=n,infers=n//10,expiries=expiry,revision=len(accepted))
   require(not (root/'.private/reader/answers.sqlite3').exists(),'noPrivateDrainDuringPublicOperations')
   if n==20:checkpoint=instance.reopen();write_json(root/'orderly_reopen.json',checkpoint)
  head=instance.head();status=rpc(instance.rcfg['socket'],{'op':'status'});verified=rpc(instance.rcfg['socket'],{'op':'verified_status'})
  require(head['revision']==verified['revision']==44 and status['received']==status['selected']==4,'publicEventCounts')
  retries=[]
  for req in [requests[0],next(r for r in requests if r['action']['kind']=='Infer')]:
  ...
  require(instance.head()==head,'historicalRetriesPreserveHead')
  ...
  require([e['ct_sha256'] for e in vstate['routes']['0']['queue']]==fresh[8:],'exactFinalOriginalQueue')
  progress('full_public_replay_started',revision=44)
  replay_start=time.perf_counter_ns();replay=instance.replay(True);replay_ns=time.perf_counter_ns()-replay_start
  require(replay['revision']==44 and replay['state_digest']==verified['state_digest'] and replay['admissions']=={'0':40,'1':0} and replay['queue_lengths']=={'0':32,'1':0},'completePublicArithmeticReplay')
  ...
  host_report=instance.worker_report;require(host_report['clean_exit'] and host_report['requests']==44 and host_report['stats']['crypto_calls']==dict(host_calls) and host_report['inspection_cache_entries']==len(cache),'persistentHostCompletedExactly44')
  host_report['cache_canonical_json_bytes']=len(canonical(cache));host_report['cache_bytes_are_not_heap_rss']=True
  public_report={'schema':'public-coin-durable-journal-public-v1','ok':True,'genesis_sha256':digest(instance.g),'context_id':instance.g['context_id'],'learns':40,'infers':4,'expiries':8,'finalized_events':44,'exact_fifo_expiry_and_final_queue':True,'ordered_authority_acceptor_histories_match':True,'independent_replay':replay,'public_replay_ns':replay_ns,'orderly_reopen':checkpoint,'authorized_historical_retries':retries,'setup':setup_result,'binding':binding,'public_log_audit':audit,'public_calls':len(calls),'native_counts':dict(counts),'role_command_costs':[{'role':r,'command':c,'count':len(xs),'sum_ns':sum(xs),'min_ns':min(xs),'max_ns':max(xs)} for (r,c),xs in sorted(costs.items())],'retained_public_cas':storage,'host_worker':host_report,'all_public_replay_and_operations_before_private_drain':True,'all_public_workers_and_services_closed':True,'private_answer_db_absent_through_public_checks':True,'public_acceptor_has_recipient_key_paths':False,'scalar_master_generated':False,'scalar_projection_delivery_generated':False,'credential_count_without_private_reads':16}
  ...
  for name in ['genesis.json','public_context.json','registry.json','public_transcript.json','context_validation.json','public_setup_verification.json','setup_report.json','public_log_audit.json','orderly_reopen.json']:
  ...
  print(json.dumps({'phase':'public_complete','learns':40,'infers':4,'expiries':8,'all_services_closed':True,'public_seconds':public_report['whole_public_phase_ns']/1e9}),flush=True)
  phase='private'
  ...
  second=private_call(root,SOURCE/'journal/drain.py','--config',root/'.private/reader/drain_config.json')
  require(first['new_private_decodes']==4 and comparison['all_match'] and second['new_private_decodes']==0 and second['already_private_decoded']==4,'normalPrivateComparisonAndDedup')
  report={'schema':'public-coin-durable-journal-normal-v1','ok':True,'learns':40,'infers':4,'exact_original_expiries':8,'finalized_events':44,'all_private_integer_comparisons_match':True,'private_integer_comparisons':4,'private_drain_retry_new_decodes':0,'genesis_sha256':public_report['genesis_sha256'],'context_id':public_report['context_id'],'public_complete_sha256':file_sha(reports/'public_complete.json'),'execution_pins_sha256':file_sha(reports/'execution_pins.json'),'whole_public_phase_ns':public_report['whole_public_phase_ns'],'all_public_operations_replay_and_services_close_before_any_private_drain':True,'scalar_master_generated':False,'scalar_projection_delivery_generated':False,'scope':['Classical DDH; honest direct public tau/U and independent honest registration; accepted values only, OS RNG state excluded.','Full per-input fixed-span recipient coalition; no selected-only release or recipient self-restraint.','Known deterministic fixture and shared OS; no private-lifetime confidentiality or isolation.','Separate Ed25519/SHA256 assumptions; no PQ or new utility result.','Aggregate correctness publication is outside the host protocol; private scores, hashes and decode durations omitted.']}
  write_json(reports/'report.json',report);print(json.dumps({'ok':True,'report':str(reports/'report.json'),'learns':40,'infers':4,'expiries':8,'all_private_integer_comparisons_match':True}),flush=True)
  ...
  private=root/'.private/postprotocol';private.mkdir(mode=0o700,exist_ok=True);write_json(private/'failure.json',{'ok':False,'error_type':type(error).__name__,'reason':str(error)},True)
  print(json.dumps({'ok':False,'phase':'private','diagnostics_retained_privately':True}),flush=True);raise SystemExit(1)

def main():
 p=argparse.ArgumentParser();p.add_argument('mode',choices=['prepare','run']);p.add_argument('--reports',type=Path,required=True);p.add_argument('--root',type=Path);a=p.parse_args();reports=a.reports.resolve()
 if a.mode=='prepare':prepare(reports)
 else:require(a.root is not None,'rootRequired');run(a.root.resolve(),reports)
