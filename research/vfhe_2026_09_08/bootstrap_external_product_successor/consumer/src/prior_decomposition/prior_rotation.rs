...
// Owned append-only public adapter. Prefix bytes are the frozen previous consumer.
pub(crate) fn verified_prefix(dir:&Path)->Result<(GlweCiphertextOwned<u32>,u32,Value)> {
 let b=bundle(dir,false)?;let prior=verify_prior(dir,&b)?;
 let (input,output)=tables(&b);let raw=read(&dir.join("rotation_template.json"),1<<20)?;
 assert_eq!(hash(&raw),"558397bfd2c7c048b7b5b375d607fadcad4150db54f6492e28281eee1a35ecbc");
 let st=statement(&dir.join("rotation_template.json"),input,output)?;
 let bytes=read(&dir.join("rotation_proof.bin"),1<<24)?;let p=proof(&bytes)?;
 let now=Instant::now();Backend::verify(&st,&p)?;let ns=now.elapsed().as_nanos();
 let row=b.prior_rows[..805].iter().find(|r|r[3]!=0).ok_or("no nonzero mask")?.clone();
 let report=json!({"modulus_switch":prior,"rotation_verified":true,"rotation_verify_ns":ns,
  "rotation_proof_sha256":hash(&bytes),"rotation_template_sha256":hash(&raw),"first_nonzero_mask_row":row});
 Ok((b.output,row[3],report))
}
