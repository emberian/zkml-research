...
h_j=g^s_j and exports no master. Its private erasure includes s, the
initializer's copies of all delivered k_i, pending/delivery records and other
master-correlated setup secrets. This is an assumption; process exit and file
permissions do not establish it.
...
integer interval. For each row, B_i=32*127*sum_j(abs(y_ij)) bounds an honest
window score and q>2*B_i makes [-B_i,B_i] injective in the group exponent.
For the recorded query rows, the largest bound is 14,219,936 in absolute value,
as computed in [the scaling evidence](../fixed_span/scaling/linear_results.json).
This bounds honest outputs, not arbitrary
...
1. [EXECUTED primitive; OPEN integration] `crypto/` owns the dedicated public CLI, private
   ...
   No previous crypto or journal implementation is edited. Its first full
   positive suite now passes 33 Learn/four exact recipient comparisons/one
   expiry and exact transition replay in 252.419 seconds. See the
   [frozen backend evidence](crypto/reports/RESULTS.md). The separately retained
   first full-validation path precedes an explicit digest-pinned validation
   cache. That cache relies on trusted validation of these exact context bytes
   and still rehashes and strictly parses them at every call.
