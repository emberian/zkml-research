...
def main():
 ...
 worker_node=next(n for n in ast.parse(original).body if isinstance(n,ast.FunctionDef) and n.name=='worker')
 worker=ast.get_source_segment(original,worker_node)
 assert '        query_index = route * 8' in worker
 worker=worker.replace('        query_index = route * 8',"        require(route == 0, 'predeclaredPublicRoute')\n        query_index = 1  # q01 fixed before the one text was encoded")
 worker=worker.replace('One private illustrative text through the already-tested plaintext SmolLM issuer frontend; no new utility estimate.',
  'One fresh private illustrative text through the fixed SmolLM3 framing-A semantic issuer; no new utility estimate.')
 worker=worker.replace('Frozen public-role snapshots; unchanged original frontend files checked before and after invocation.',
  'Frozen public-role snapshots and new bounded frontend wiring; actual scorer tensor loop unchanged; source pins checked before and after.')
 adapted=original[:original.index('def stage(args):')]+worker+'''
def main():
...
'''
 ...
 assert live_class(original)==live_class(adapted)
 dest=SNAP/'verified_reader/live_driver.py';dest.write_text(adapted);pins['verified_reader/live_driver.py']=sha(dest)
 (ROOT/'live_driver_adapter.diff').write_text('\n'.join(difflib.unified_diff(original.splitlines(),adapted.splitlines(),fromfile='frozen_original_live_driver.py',tofile='semantic_live_driver.py'))+'\n')
 ...
 for p in [ROOT/n for n in ['CONTRACT.md','.gitignore','prepare.py','run.py','validate.py','issue_semantic_text.py','encoder/common.py','encoder/score.py','encoder_inventory.json']]:deps[str(p)]=sha(p)
 ...
 freeze={'public_source_dependencies':deps,'snapshot_source_pins':pins,'query_vector_pins':querypins,
  ...
  'source_model_forwards_before_freeze':0,'crypto_calls_before_freeze':0,'private_input_prompt_freeze_stored':True,
