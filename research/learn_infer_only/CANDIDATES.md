...
[DERIVED current comparison, renewed constructive swarm] A useful continuing text learner now has a persistent 78-class encrypted model, and its actual expiry reaches a Lean-generated proof and native/WebAssembly verification. The fixed-coordinate ring construction is now implemented and its faster full-size run takes 117.13 seconds; the earlier “no implementation” statement below is historical. A bounded nonlinear MIFE Replay construction adds a conditional fresh-input privacy argument. These advances still belong to distinct objects: the useful BFV learner retains a full reader; ring recipients retain their fixed-span read powers; the MIFE executable is symbolic. The next builders target actual ct×ct rescale, nonlinear encrypted utility, proof-gated durable updates, multi-round FRI and separate-process ring transport. See [the current construction status](../vfhe_2026_09_08/STATUS.md).
