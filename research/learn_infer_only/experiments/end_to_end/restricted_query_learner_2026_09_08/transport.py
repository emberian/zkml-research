...
def basic_header(profile,kind,setup_id=None,**extra):
    return {'format_version':2,'registry_sha256':BASIS_SHA256,'kind':kind,'profile':profile,'parameters':asdict(params(profile)),
...
def validate_header(h):
    common={'format_version','registry_sha256','kind','profile','parameters','setup_id'}
    ...
    if type(h['format_version']) is not int or h['format_version']!=2 or h['profile']!='candidate_full':
    ...
    for name in ['setup_id','a_sha256','public_sha256','registry_sha256']:
...
def context_equal(a,b):
    for name in ['profile','setup_id','parameters','a_sha256','registry_sha256']:
...
def command_init(a):
    p=params(a.profile); public=PublicSetup(p,None); public.basis=BASIS
...
def command_finalize(a):
    ...
    setup.basis=BASIS; setup.sampler=None
...
def command_encode(a):
    ...
    if any(abs(v if v<=public.params.p//2 else v-public.params.p)>127 for v in x) or x[-1]!=0:raise ValueError('semantic issuer expects int8 feature residues and final zero')
    ct=public.encode(x)
...
def main():
    ...
    receipt={'status':status,'role_command':a.command,'pid':os.getpid(),'elapsed_seconds':time.perf_counter()-start,
             ...
             'registry_sha256':BASIS_SHA256,'artifact_reads':READS,'artifact_writes':WRITES,'result':result,'error':error}
