...
ACTION_BASE=['schema','genesis','params_id','program_id','program_version','key_id','public_key_sha256','context_id','kind','route','request_id','nonce','parent_revision','parent_state','recipient']

def validate_action(action,authorization,g,cas):
 require(isinstance(action,dict),'actionObject');kind=action.get('kind')
 extra=['record_id','fresh_ct','feature_policy','range_assertion'] if kind=='Learn' else ['query_ct','row_id','row_sha256','token_sha256']
 ...
 require(integer(action['program_version']) and action['program_version']==1,'programVersionType')
 ...
 if kind=='Learn':
  require(action['recipient']==g['recipient'],'learnRecipientCoalition')
  require(action['feature_policy']==g['feature_policy'] and canonical(action['range_assertion'])==canonical([-127,127]),'issuerPolicy')
  ...
  for key in ['row_id','row_sha256','token_sha256']:require(action[key]==binding[key],'actionDesignatedBinding:'+key)
  require(action['recipient']==binding['recipient_id'],'actionDesignatedRecipient')
 require(canonical(payload)==canonical(action),'authorizationContext')
 return kind
