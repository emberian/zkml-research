...
// Fixed public prime already recorded by the original params command.
fn t()->u64{4294475777}
...
fn execute(command:String,a:BTreeMap<String,String>,p:Arc<BfvParameters>,cached_ek:Option<&EvaluationKey>)->Res<Value>{
 let get=|k:&str|->Res<&str>{a.get(k).map(String::as_str).ok_or_else(||format!("missing{k}").into())};let start=Instant::now();
 let mut result=match command.as_str(){
  ...
  "infer"=>{let dir=Path::new(get("--dir")?);let owned_ek;let ek=if let Some(k)=cached_ek{k}else{owned_ek=EvaluationKey::from_bytes(&read(dir.join("evaluation.key").to_str().unwrap())?,&p)?;&owned_ek};let ct=loadct(get("--acc")?,&p,2)?;let query=vecfile(get("--query")?)?;let mut slots=vec![0u64;N];for(i,x)in query.iter().enumerate(){for lane in 0..8{slots[i*8+lane]=residue(*x)}}let pt=Plaintext::try_encode(&slots,Encoding::simd(),&p)?;
...
fn serve()->Res<()> {
 use std::io::BufRead;
 let args:Vec<String>=std::env::args().collect();if args.len()!=4||args[1]!="serve"||args[2]!="--dir"{return Err("serve --dir MODEL required".into())}
 let dir=Path::new(&args[3]);let p=params()?;let ek=EvaluationKey::from_bytes(&read(dir.join("evaluation.key").to_str().unwrap())?,&p)?;
 println!("{}",json!({"ready":true,"public_only":true,"parameters":desc(&p)}));std::io::stdout().flush()?;
 for line in std::io::stdin().lock().lines(){
  let line=line?;let result=(||->Res<Value>{let request:Value=serde_json::from_str(&line)?;let command=request["command"].as_str().ok_or("command required")?;
   if command!="infer"&&command!="learn"{return Err("public host supports infer/learn only".into())}
   let mut values=BTreeMap::new();for(k,v)in request["arguments"].as_object().ok_or("arguments required")?{values.insert(format!("--{k}"),v.as_str().ok_or("string argument required")?.to_string());}
   if let Some(request_dir)=values.get("--dir"){if Path::new(request_dir)!=dir{return Err("fixed public-key directory".into())}}
   execute(command.to_string(),values,p.clone(),Some(&ek))})();
  match result{Ok(value)=>println!("{}",json!({"ok":true,"result":value})),Err(error)=>println!("{}",json!({"ok":false,"error":error.to_string()}))}
  std::io::stdout().flush()?;
 }
 Ok(())
}
fn main(){if let Err(error)=serve(){eprintln!("{}",json!({"error":error.to_string()}));std::process::exit(2)}}
