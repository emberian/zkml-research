...
def public_run(root,reports):
 ...
 try:
  ...
  for n in range(1,34):
   ...
   if n==16:checkpoint=instance.reopen();write_json(root/'orderly_reopen.json',checkpoint)
  ...
  final_setup=subprocess.run([sys.executable,str(SOURCE/'seed_setup/seed_setup.py'),'verify-public','--registry',str(root/'registry.json'),'--registry-sha256',digest(read_json(root/'registry.json')),'--context',str(root/'public_context.json'),'--transcript',str(root/'public_transcript.json')],capture_output=True,timeout=300)
  require(final_setup.returncode==0,'finalCompleteSeedReplay');write_json(root/'final_seed_replay.json',{'argv':[sys.executable,str(SOURCE/'seed_setup/seed_setup.py'),'verify-public'],'exit_code':final_setup.returncode,'stdout':final_setup.stdout.decode(),'stderr':final_setup.stderr.decode()})
...
def main():
 ...
 if a.mode=='prepare':prepare(reports)
  ...
  require((reports/'LAUNCH.json').exists(),'reviewedLauncherRequired')
