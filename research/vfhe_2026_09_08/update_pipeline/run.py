#!/usr/bin/env python3
"""Four public BFV ciphertexts -> Lean witness -> actual independently verified proof."""
...
def main() -> int:
    ...
    parser.add_argument("--lean", help="Lean 4.30 executable; default is the recorded local compiler")
    parser.add_argument("--lean-path", help="compiled dependency search path; default is the recorded local overlay")
    parser.add_argument("--threads", type=int, default=4)
    parser.add_argument("--checked-export", action="store_true", help="also interpret-check every source constraint in Lean before proving")
    ...
    context = json.loads((ARITHMETIC / "results/checked_modules.json").read_text())
    lean = args.lean or context["lean"]
    lean_path = args.lean_path or context["lean_path"]
    emitter = ARITHMETIC / "EmitBfvExpiry.lean" if args.checked_export else PACKAGE / "EmitBfvExpiryFast.lean"
    if not Path(lean).is_file() and not shutil.which(lean):
        parser.error("Lean executable unavailable; set --lean and --lean-path")
    for name in NAMES:
    ...
    report = {
        ...
        "compiler_source_sha256": sha256(emitter),
        "checked_export": args.checked_export,
        "lean": lean, "lean_path": lean_path,
        "inputs": {},
    ...
    try:
        ...
        run("emit", [lean, "--run", str(emitter), str(emitted),
