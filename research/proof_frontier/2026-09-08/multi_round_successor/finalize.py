...
repo=Path('/tmp/minidregg-multi-round-successor-20260908')
research=out.parent
...
patch=out/'arity-eight-two-round.patch';chunks=[]
...
replay=Path(tempfile.mkdtemp(prefix='arity-eight-two-round-replay-'))
...
pred=research/'constructive_successor';pm=json.loads((pred/'MANIFEST.json').read_text())
for row in pm['modules']:
 assert sha(pred/'src'/row['path'])==row['sha256']==sha(repo/row['path'])
...
entry={'name':'multi_round_successor','root':'../proof_frontier/2026-09-08/multi_round_successor','patch':'../proof_frontier/2026-09-08/multi_round_successor/arity-eight-two-round.patch','expected_pins':sum(x['guards']for x in modules),'source_files':{str(m):'../proof_frontier/2026-09-08/multi_round_successor/src/'+str(m)for m in sorted(mods)}}
save(out/'integration_entry.json',entry)
(out/'STATUS.md').write_text("""[DERIVED] Complete: two actual arity-eight transitions compose into terminal RS-membership soundness over the existing coherent sampler. Prefix-dependent second input and middle commitments are allowed; the proof derives and sums conditional challenge failures, then pays one sampling tail.

[DERIVED] The supplied-opening theorem uses the same extracted middle checkpoint in both rounds and retains the observed finite-log Failure residual. Its actual BabyBearExt4 specialization is `(2^20+2^17)/2013265921^4 + (9/10)^q + Pr[Failure]`, with the certified `2^20→2^17→2^14` tower and degrees `2^19→2^16→2^13`.

[DERIVED] The 128→16→2 schedule has a provably far initial word, a second injected word that actually depends on the first scalar, accepted zero seeds and rejected seed one. A separate complete supplied-path/log witness inhabits source extraction, farness, checkpoint and supplied-path inclusion, acceptance and zero observed failure.

[EXECUTED] Five additive modules, 43 exact theorem axiom guards, five final changed-module checks (exit 0, empty output), import boundary and exact patch replay pass. Main and frozen one-round/curve/kernel/carrier/tower dependencies are unchanged. No broad rebuild or new independent-review campaign was run. See MANIFEST.json and checks/.

[OPEN] This is a terminal-codeword theorem for two external rounds. Runtime packed MMCS, actual terminal-polynomial equivalence, transcript/log completeness and cryptographic pricing of observed Failure remain explicit boundaries. No whole-protocol or security-bit budget is added. Root owns integration.
""")
manifest={'scope':'two fresh-scalar arity-eight terminal transitions; existing coherent sampler; extracted supplied BinaryMerkle openings','base_commit':base,'checkout':str(repo),'lean_toolchain':(repo/'lean-toolchain').read_text().strip(),'modules':modules,'module_count':len(modules),'theorem_count':sum(m['theorems']for m in modules),'all_exact_guarded_checks_pass':True,'standard_axioms_only':True,'patch':patch.name,'patch_sha256':sha(patch),'dependencies':deps,'source_evidence':evidence,'import_gate':ig,'patch_replay':pr,'broad_rebuild_performed':False,'independent_review_campaign_performed':False,'new_web_queries':0,'new_scry_sql_queries':0,'predecessors_preserved':True,'artifact_sha256':{str(p.relative_to(out)):sha(p)for p in [out/'README.md',out/'STATUS.md',out/'NEXT.md',out/'DEPENDENCIES.json',out/'declaration-names.json',out/'integration_entry.json',out/'run_lean.py',out/'pin_all.py',out/'finalize.py']}}
