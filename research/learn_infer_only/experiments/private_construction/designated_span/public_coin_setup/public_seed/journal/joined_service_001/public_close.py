...
def close(root,reports):
 ...
 closed_log_audit=check_public_logs(root)
 for compressed in reports.glob('*.gz'):
  require(gzip.decompress(compressed.read_bytes())==(root/compressed.name[:-3]).read_bytes(),'closedPublicLogSnapshot:'+compressed.name)
