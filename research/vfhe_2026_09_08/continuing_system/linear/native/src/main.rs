//! Complete caller-selected linear Infer: public trace binding, generated MAC proofs.
...
use fhe::bfv::{
    BfvParameters, BfvParametersBuilder, Ciphertext, Encoding, EvaluationKey, Plaintext, SecretKey,
};
use fhe_math::rq::{Poly, Representation, traits::TryConvertFrom};
use fhe_traits::{DeserializeParametrized, FheEncoder, FheDecoder, FheDecrypter, Serialize};
use ndarray::Array2;
...
fn binding(case: &Path, plan_path: &Path) -> Result<Value> {
    ...
    Ok(
        json!({"linear_plan_sha256":hash(&read(plan_path,4<<20)?),"model_ciphertext_sha256":hash(&model),"query_sha256":hash(&q),"evaluation_key_sha256":hash(&key),"dot_ciphertext_sha256":hash(&dot)}),
    )
}
...
fn import(
    ...
    fs::write(out.join("expected_dot.ct"), current.to_bytes())?;
    ...
    let report = json!({"claim":"EXECUTED complete caller-selected packed linear dot/reduction public export","N":N,"moduli":Q,"t":T,"stages":11,"chunks":chunks,"public_arity":ARITY,"mac_tuple_rows":11*4*N,"output_residues":2*11*4*N,"final_step_dot_bytes_equal":true,"key_branches":key_meta,"binding":binding(out, plan_path)?,"private_files_read":0,"square_executed":false,"ct_ct_multiplications":0,"elapsed_ns":timer.elapsed().as_nanos(),"verifier_tcb":"Public SIMD encoder, canonical residue selection/lift, NTT/permutation and affine additions, public key decoder/seed expansion. Generated proof enforces all products and switching sums."});
...
fn read_dot(issuer: &Path, dot_path: &Path, expected_dot: &str, expected_key: &str) -> Result<()> {
    let p = params()?;
    let public_key_bytes = read(&issuer.join("evaluation.key"), 16 << 20)?;
    assert_eq!(hash(&public_key_bytes), expected_key);
    let encoded = read(dot_path, 2 << 20)?;
    assert_eq!(hash(&encoded), expected_dot);
    let ciphertext = Ciphertext::from_bytes(&encoded, &p)?;
    assert_eq!(ciphertext.len(), 2);
    assert_eq!(ciphertext.to_bytes(), encoded);
    ...
    let residues = &values[..8];
    assert!(values.iter().enumerate().all(|(i, x)| *x == residues[i % 8]));
    let dots: Vec<i64> = residues.iter().map(|&x| if x > T / 2 { x as i64 - T as i64 } else { x as i64 }).collect();
    assert!(dots.iter().all(|x| (-20000..=20000).contains(x)));
    let total: i64 = dots.iter().sum();
    println!("{}", json!({"dot_ciphertext_sha256":expected_dot,"evaluation_key_sha256":expected_key,
        "dot_values":dots,"sum_dot":total,"all8192_slots_repeat8":true,"signed_bound":20000,
        "full_reader_key":true,"scope":"Bounded signed BFV dot slots; full reader capability remains. Proof/controller acceptance is the caller's responsibility."}));
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    match a.get(1).map(String::as_str) {
        Some("import") if a.len() == 7 => import(Path::new(&a[2]), Path::new(&a[3]), Path::new(&a[4]), Path::new(&a[5]), Path::new(&a[6])),
        ...
        Some("read") if a.len() == 6 => read_dot(Path::new(&a[2]), Path::new(&a[3]), &a[4], &a[5]),
        _ => Err("usage: import LINEAR_PLAN MODEL QUERY EVALKEY NEW_CASE | prove LINEAR_PLAN INDEX TEMPLATE CASE TRACE NEW_PROOF | verify LINEAR_PLAN INDEX TEMPLATE CASE PROOF | read ISSUER DOT EXPECTED_DOT_SHA256 EXPECTED_EVALKEY_SHA256".into())
    }
}
