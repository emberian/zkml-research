...
def crash(point,selected,marker):
 if point==selected:
  ...
  # SIGKILL delivery can be asynchronous; never return to publication/reply code.
...
class Authority:
 ...
 def publish(self,envelope):
  # Normal ordered delivery first supplies public encrypted material, then asks
  # the recipient to recompute the entire transition before accepting its output.
  p=envelope['payload'];a=p['request']['action']
  try:
   blobs=[(p['request']['proposal']['result_ct'],'ct')]
   blobs.append((a['fresh_ct'],'ct') if a['kind']=='Learn' else (a['query_ct'],'query'))
   for hexdigest,kind in blobs:
    raw=self.cas.get(hexdigest,kind).read_bytes()
    ack=rpc(self.cfg['reader_socket'],{'op':'put','kind':kind,'sha256':hexdigest,'data':base64.b64encode(raw).decode()})
    require(ack.get('ok'),'readerBlobAcceptance')
   checked=rpc(self.cfg['reader_socket'],{'op':'sync','envelope':envelope})
   if not checked.get('ok'):return {'status':'pending','reader':checked}
   if a['kind']=='Learn':return {'status':'verified_not_a_release','verification':checked}
   ...
   if ack.get('ok'):
    ...
    return {'status':'acknowledged','meaning':'verified_ciphertext_durably_accepted_no_decode','reader':ack,'verification':checked}
 ...
 def submit(self,req):
  ...
  try:
   ...
   if row:
    ...
    require(canonical(req['proposal'])==canonical(expected),'recomputedProposalMismatch')
 ...
 def handle(self,msg):
  ...
  if op=='export':
   ...
   with connect(self.cfg['db']) as db:
    db.execute('BEGIN')
    rows=db.execute('SELECT envelope,published FROM journal ORDER BY revision').fetchall()
    meta=db.execute('SELECT * FROM meta').fetchone()
    db.execute('COMMIT')
   state=json.loads(meta['state']);require(validate_state(state,self.g,self.cas)==meta['state_digest'],'exportStateDigest')
   head={'revision':meta['revision'],'state_digest':meta['state_digest'],'state':state}
   return {'ok':True,'head':head,'journal':[{'envelope':json.loads(x['envelope']),'published':bool(x['published'])} for x in rows]}
...
class Handler(socketserver.StreamRequestHandler):
 def handle(self):
  try:
   line=self.rfile.readline(2_000_001);require(len(line)<=2_000_000,'requestSize');response=self.server.role.handle(parse_json(line))
