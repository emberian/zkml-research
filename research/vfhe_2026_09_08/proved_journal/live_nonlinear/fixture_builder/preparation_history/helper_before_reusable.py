...
def encode(out):
    """The only model invocation: fresh cache, exactly one batch of three texts."""
    ...
    import numpy as np
    encoder = module.Encoder(cache=out / 'encoder_cache')
    with contextlib.redirect_stdout(sys.stderr):
        raw = encoder.encode([entry['text'] for entry in entries])
    vectors = np.rint(np.asarray(raw, dtype=np.float64)[:, :576] / 4).astype(np.int64)
    if vectors.shape != (3,576) or np.max(abs(vectors)) > 32 or np.any(np.sum(vectors*vectors,axis=1) > 20000):
        raise RuntimeError('fixed feature domain failed; no retuning or retry')
    if encoder.stats['forward_batches'] != 1 or encoder.stats['encoded_texts'] != 3 or encoder.stats['cached_texts'] != 0:
        raise RuntimeError('expected exactly one fresh batch of three texts')
    rows = []
    for entry, vector in zip(entries, vectors):
        path = out / (entry['id'] + '.json')
        write_new(path, vector.tolist())
        rows.append({'id':entry['id'], 'vector_path':str(path), 'sha256':sha(path),
                     'coordinate_max_abs':int(np.max(abs(vector))),
                     'norm_squared':int(vector @ vector)})
    record = {'fixture_sha256':sha(HERE/'fixture.json'), 'encoder_stats':encoder.stats,
