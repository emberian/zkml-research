...
prior=json.loads((p.parent/'formal/results/RangeKeyswitchInfer_003.json').read_text())
env=dict(os.environ,LEAN_PATH=str(p/'build')+':'+str(p.parent/'backend/build')+':'+prior['LEAN_PATH'])
for srcdir in [p.parent/'formal/build/Compiler',p.parent/'backend/build/Compiler']:
