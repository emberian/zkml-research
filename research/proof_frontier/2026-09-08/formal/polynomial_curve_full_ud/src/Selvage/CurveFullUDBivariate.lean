...
-/
import Selvage.CurveFullUDInterpolation
...
theorem curve_bw_bivariate_of_many_close
    (M e deg : ℕ) (domain : ι ↪ F) (u : Fin (M+1) → ι → F) (good : Finset F)
    (hM : 1 ≤ M) (hd : 1 ≤ deg) (hradius : 2*e+deg ≤ Fintype.card ι)
    (hcard : M*Fintype.card ι < good.card)
    (hclose : ∀ z ∈ good, ∃ w ∈ reedSolomonCode domain deg,
      hammingDist (curveWord u z) w ≤ e) :
    ∃ A B : F[X][Y], A ≠ 0 ∧ degreeX A ≤ e ∧ natDegreeY A ≤ M*e ∧
      degreeX B ≤ e+deg-1 ∧ natDegreeY B ≤ M*(e+1) ∧
      (∀ i, evalX (domain i) B =
        (curvePolynomial u i) *
          evalX (domain i) A) := by
  classical
  obtain ⟨a,b,ha_ne,ha_deg,hb_deg,hMul⟩ :=
    curve_bw_bounded_kernel_of_many_close M e deg domain u good hM hd hradius hcard hclose
  ...
  · -- natDegreeY A bound
    have hdegX_A0 : Polynomial.Bivariate.degreeX A0 ≤ M*e := by
      classical
  ...
  · -- natDegreeY B bound
    have hdegX_B0 : Polynomial.Bivariate.degreeX B0 ≤ M*(e + 1) := by
      classical
      unfold Polynomial.Bivariate.degreeX
      refine Finset.sup_le_iff.2 ?_
      intro n hn
...
/-- info: 'Minidregg.Selvage.curve_bw_bivariate_of_many_close' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Minidregg.Selvage.curve_bw_bivariate_of_many_close
