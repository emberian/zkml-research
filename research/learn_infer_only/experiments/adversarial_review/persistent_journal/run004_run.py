...
class Case:
 def __init__(self,name):
  ...
  self.r=self.start('recipient');self.a=self.start('authority');CASES.append(self)
 ...
 def start(self,role,socket_override=None):
 ...
 def launch_client(self,key='primary',failpoint=None,request=None,barrier=None,host=True,socket_override=None):
  ...
  if failpoint:cmd+=['--failpoint',failpoint,'--crash-marker',self.path/f'{failpoint}.marker.json']
...
def crash_case(stage,committed,published):
 c=Case(stage);answer=c.client(failpoint=stage)
...
def publication_before_commit_control():
 c=Case('broken_publish_before_commit');answer=c.client(failpoint='broken_publish_before_commit')
...
def retry_and_mutation_case():
 c=Case('retry_and_exact_packet');first=c.client();assert first['status']=='accepted'
 ...
 c.close();return dict(case=c.name,concurrent_exact_retries=8,changed_identity_refusals=refusals,logical_journal_packets=1,recipient_deliveries=1,publication_attempts=1)
