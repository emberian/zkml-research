...
// Append-only adapter to the unchanged predecessor consumer.
pub(crate) fn verified_input(dir:&Path)->Result<(GlweCiphertextOwned<u32>,Vec<Vec<u32>>,serde_json::Value)> {
 let (ct0,a,prefix)=prior_rotation::verified_prefix(dir)?;
 let delta:GlweCiphertextOwned<u32>=decode(&dir.join("delta.ct"))?;
 assert_eq!(delta.polynomial_size().0,512);assert_eq!(delta.glwe_size().0,4);assert!(delta.ciphertext_modulus().is_native_modulus());
 let digits:Vec<Vec<u32>>=serde_json::from_slice(&read(&dir.join("digits.json"),1<<20)?)?;
 assert_eq!(digits.len(),2048);assert!(digits.iter().all(|d|d.len()==4&&d[0]<1024&&d[1]<1024&&d[2]<=1024&&d[3]<=1024));
 let (input,output)=tables(&ct0,&delta,a,&digits);
 let raw=read(&dir.join("decomposition_template.json"),1<<20)?;
 assert_eq!(hash(&raw),"d1306386767a9ec8bdfb9bddf3e0b2f831b352feafdbd3b5e2572baa7aa4c52a");
 let st=statement(&dir.join("decomposition_template.json"),input,output)?;
 let bytes=read(&dir.join("decomposition_proof.bin"),1<<24)?;let p=proof(&bytes)?;
 let now=Instant::now();Backend::verify(&st,&p)?;
 let report=json!({"prefix":prefix,"decomposition_verified":true,"decomposition_verify_ns":now.elapsed().as_nanos(),"decomposition_proof_sha256":hash(&bytes),"decomposition_template_sha256":hash(&raw)});
 Ok((delta,digits,report))
}
