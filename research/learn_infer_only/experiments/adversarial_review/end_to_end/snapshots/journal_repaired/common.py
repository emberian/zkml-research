"""Public encodings, Ed25519 role credentials, strict immutable blobs and BFV CLI."""
...
def identity_fields(genesis):
 return {'genesis':checked_genesis(genesis),'params_id':genesis['crypto']['params_id'],
  ...
  'public_key_sha256':genesis['public_key_sha256']}
def rpc(address,obj,timeout=90):
...
class Crypto:
 ...
 def run(self,command,private_output=False,**kwargs):
  ...
  start=time.perf_counter_ns();out=subprocess.run(argv,capture_output=True,timeout=90)
  elapsed=time.perf_counter_ns()-start
  # Reader scalar output is never written to a public command log.
  record={'role':self.role,'command':argv,'elapsed_ns':elapsed,'exit_code':out.returncode,
...
class CAS:
 ...
 def get(self,hexdigest,kind='ct'):
  ...
  if kind=='ct':
   ...
   exact_keys(query,['schema','params_id','coefficients'],'queryFields')
   require(query['schema']=='resident-public-query-v1' and query['params_id']==self.genesis['crypto']['params_id'],'queryParams')
