...
def main():
    ...
    sources=[HERE/'transport.py',HERE/'workflow.py',HERE/'codec/packed.py']
    before={str(path):digest(path) for path in sources}

    def actor(label,role,args):
        receipt=results/(label+'.json')
        command=['/usr/bin/sandbox-exec','-f',str(roleprofiles[role]),sys.executable,'-B',str(HERE/'transport.py'),
    ...
    try:
        actor('init','public',['init','--profile',a.profile,'--out',public/'a.ring'])
        for i in range(p.recipients):
            actor(f'register_{i:02d}',f'recipient_{i:02d}',[
                'register','--public-a',public/'a.ring','--coordinate',i,
                '--key-out',recipients[i]/'key.ring','--registration-out',public/f'registration_{i:02d}.ring'])
        finalargs=['finalize','--public-a',public/'a.ring','--out',public/'public.ring']
        ...
        for step in range(3):
            ...
            actor(f'encode_{step}','public',['encode','--public',public/'public.ring','--input',ip,'--out',public/f'cipher_{step}.ring'])
        ...
        result={'status':'PASS','started_utc':started,'finished_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
                ...
                'orchestrator_private_payload_reads':0,'security_scope':'Experiment artifact path sandbox; same-host administration and malicious-process sandbox escapes not addressed'}
