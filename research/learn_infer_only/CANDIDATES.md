...
[DERIVED current comparison after the architectural reset] The useful BFV learner retains a full reader. The restricted-key ring learner avoids constructing a universal reader, but its fixed projections expose the state needed to continue its present additive behavior. These are separate partial capabilities, not a completed private resident. The new rolling bilinear candidate fails more strongly: the actual surviving writer credentials reveal all576 variable semantic coordinates through permitted probes. The inspected noisy-FE alternatives do not remove this capability; fixed key noise cancels between probes. See the [source algorithms and exact exposure argument](experiments/private_construction/release_successor_2026_09_08/INTERACTION.md).

[EXECUTED/OPEN current construction] One complete saved nonlinear kernel Infer now passes all116 proof checks in one joined verifier. Its earlier model history and full BFV reader remain explicit boundaries. The row-level Lean composition and cumulative integration patch are being sealed, while a separate builder connects soundness to the actual IR2 verifier. The fresh dual384-bit seeded semantic service is complete under its conditional security model. The restricted-ring129-proof expansion, new full-PBS setup and failed TensorSketch adapter are paused. [Current outcomes and scopes](../vfhe_2026_09_08/STATUS.md) supersede historical activity below.
