#!/usr/bin/env python3
"""Emit the unchanged expiry/update witness using the selected Lean-generated native plan."""
...
def main():
 ...
 for name,digest in pins['shared_files'].items():
  if sha(Path(name))!=digest:raise SystemExit('selected shared file changed: '+name)
 cmd=[str(P.parent/'query_native_emitter/native/target/release/lean-query-witness'),str(P/'program/witness_plan.json'),str(P/'program/template_ir2.json'),str(rows),str(out)]
