...
class AuthorityHandler(socketserver.StreamRequestHandler):
 ...
 def submit(self,message):
  ...
  crash_at(message,'before_reservation')
  db=connect(self.server.database)
  try:
   db.execute('BEGIN IMMEDIATE')
   ...
   if row is not None:
    ...
    crash_at(message,'after_install_before_commit')
    ...
    if message.get('failpoint')=='broken_publish_before_commit':
     rpc(self.server.recipient,{'nonce':i['nullifiers'][0],
       'transaction_id':txid,'packet':i['event']})
     crash_at(message,'broken_publish_before_commit')
    ...
    crash_at(message,'after_commit_before_publication')
   ...
   if row['published']:
    ...
    crash_at(message,'after_publication_before_client_ack')
...
def client(args):
 ...
 if args.failpoint:payload.update(failpoint=args.failpoint,crash_marker=args.crash_marker)
...
def main():
 ...
 s=subs.add_parser('serve');s.add_argument('role',choices=['authority','recipient']);s.add_argument('--socket',required=True);s.add_argument('--database',required=True);s.add_argument('--recipient');s.add_argument('--fixture')
 c=subs.add_parser('client');c.add_argument('--socket',required=True);c.add_argument('--request',required=True);c.add_argument('--output',required=True);c.add_argument('--barrier');c.add_argument('--failpoint');c.add_argument('--crash-marker');c.add_argument('--host-state')
 args=p.parse_args()
