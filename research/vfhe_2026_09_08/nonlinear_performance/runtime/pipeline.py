#!/usr/bin/env python3
"""Caller-selected compact-range update and kernel proofs under one approved profile.
...
def config():
    if not __debug__:
        raise RuntimeError('This fail-closed driver requires Python assertions enabled')
    c = read(ROOT / 'PIPELINE.json')
    assert c['schema'] == 'nonlinear-range-approved-profile-v1'
    check_pins(c['pins'])
    return c
...
def check_record(c, kind, i, case, record, proof):
    ...
    assert record['backend'] == c['backend_id']
